...
$sql = "SELECT C.id_candidato, C.nome, C.cognome FROM candidati AS C\n"
...
<body>
    ...
    <center>
        ...
        <form action="" method="POST" name="candidato" id="selectCandidato">
            <select name="candidato" class="form-select w-25 mt-3" aria-label="Default select example">
                <option selected>- - - - - - -</option>

                <?php
                while ($row = $result->fetch_assoc()) {
                    echo '<option name="candidato" value="' . $row["C.id_candidato"] . '">' . $row["cognome"] . " " . $row["nome"] . '</option>';
                }
                ?>
